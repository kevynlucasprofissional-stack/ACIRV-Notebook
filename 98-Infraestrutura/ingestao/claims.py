...
Uso como módulo:
    from claims import Claim, load_claims, save_claims, validate_claims_dataset, reconcile_claims_with_ledger

Uso direto (validar arquivo de claims contra o vault):
    python claims.py <vault_root> [claims_file] [ledger_file]
"""
...
def validate_claim_schema(claim: dict) -> list[str]:
    ...
    # Claim promovido deve ter fontes de suporte primárias
...
def validate_claims_dataset(
    ...
    for idx, claim in enumerate(claims, 1):
        ...
        # 2. Rastreabilidade com o ledger (supporting_sources)
        ...
        # 3. Validação do arquivo canônico de destino (canonical_destination)
        ...
        if cdest:
            ...
            if not dest_path.exists():
                ...
                # 4. Validação de anchor se declarado
        ...
        # 5. Integridade de relações (supersedes, contradicts, duplicates)
        ...
        # 6. Invariante de pending_validation em nota canônica
        if claim.get("disposition") == "promoted" and claim.get("validation_status") == "pending_validation":
            if cdest and (vault_root / cdest).exists():
                content = (vault_root / cdest).read_text(encoding="utf-8", errors="replace")
                if "pending_validation" not in content and "provisório" not in content.lower() and "pendência" not in content.lower():
                    errors.append(
                        f"Claim '{cid}': promovido como 'pending_validation', mas a nota '{cdest}' "
                        f"não o identifica como provisório/pendente de validação!"
                    )

    return errors


def reconcile_claims_with_ledger(
    vault_root: Path,
    ledger_path: Path,
    claims_path: Path,
) -> dict:
    """Fecha o ciclo registrando no ledger o resultado do processamento de claims por fonte."""
    from inventariar_fontes import load_ledger, save_ledger

    ledger_index = load_ledger(ledger_path)
    claims = load_claims(claims_path)

    now_iso = datetime.now(timezone.utc).isoformat()
    stats = {"sources_updated": 0, "errors": []}

    # Agrupa claims por (source_path, source_blob_sha)
    claims_by_source: dict[tuple[str, str], list[dict]] = {}
    for c in claims:
        for src in c.get("supporting_sources") or []:
            key = (src.get("source_path", ""), src.get("source_blob_sha", ""))
            if key[0] and key[1]:
                claims_by_source.setdefault(key, []).append(c)

    # Reconcilia cada fonte mencionada nos claims com o ledger
    for key, source_claims in claims_by_source.items():
        if key not in ledger_index:
            stats["errors"].append(
                f"Erro de reconciliação: fonte ({key[0]}, sha={key[1][:8]}) possui claims mas não existe no ledger!"
            )
            continue

        entry = ledger_index[key]

        dispositions = [c.get("disposition", "unknown") for c in source_claims]
        from collections import Counter
        disp_counts = dict(Counter(dispositions))

        # Uma fonte fica 'processed' se todos os seus claims extraídos têm disposition explícita diferente de pending_validation pura
        all_explicit = all(d in VALID_DISPOSITIONS and d != "pending_validation" for d in dispositions)
        new_status = "processed" if all_explicit else "partially_processed"

        entry["processing_status"] = new_status
        entry["read_status"] = "processed"
        entry["last_processed"] = now_iso
        entry["processor_version"] = "claims.py/2.0"
        entry["claims_detected"] = len(source_claims)
        entry["claims_by_disposition"] = disp_counts
        entry["processing_result"] = f"Reconciliado com {len(source_claims)} claim(s) canônico(s)"

        stats["sources_updated"] += 1

    save_ledger(ledger_path, list(ledger_index.values()))
    return stats
...
def main() -> int:
    ...
    parser.add_argument("--reconcile", action="store_true", help="Atualiza status no ledger para fontes com claims")
    ...
    if args.reconcile and ledger_path.exists():
        r_stats = reconcile_claims_with_ledger(vault_root, ledger_path, claims_path)
        print(f"Reconciliação com ledger efetuada: {r_stats['sources_updated']} fonte(s) atualizada(s).")
