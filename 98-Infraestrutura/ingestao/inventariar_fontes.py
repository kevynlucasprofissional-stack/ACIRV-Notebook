...
def git_ls_files_with_sha(vault_root: Path) -> list[tuple[str, str]]:
    ...
    for line in result.stdout.splitlines():
        ...
        if len(parts) == 2:
            ...
            # Normalizar separadores para barra normal
...
def load_ledger(ledger_path: Path) -> dict[tuple[str, str], dict]:
    ...
    Lança ValueError explícito em caso de erro de parsing JSON ou violação de schema.
    """
    ...
    with ledger_path.open(encoding="utf-8") as f:
        for i, line in enumerate(f, 1):
            ...
            if key in index:
                raise ValueError(
                    f"Entrada duplicada para (source_path='{source_path}', blob_sha='{blob_sha}') no ledger '{ledger_path}', linha {i}"
                )
            index[key] = entry
...
def save_ledger(ledger_path: Path, entries: list[dict]) -> None:
    ...
    with ledger_path.open("w", encoding="utf-8", newline="\n") as f:
        for entry in sorted(entries, key=lambda e: (e.get("source_path", ""), e.get("blob_sha", ""))):
...
def run(vault_root: Path, ledger_path: Path, verbose: bool = False, with_commits: bool = False) -> dict:
    ...
    # Carrega ledger existente com validação estrita
    existing = load_ledger(ledger_path)

    # Obtém lista atual de arquivos com blob SHA via Git
    ...
    # 1. Detecta remoções: entradas ativas no ledger cujo arquivo não existe mais no Git
    ...
    for blob_sha, source_path in git_entries:
        ...
        if key in existing:
            ...
            new_ledger_map[key] = entry
            if entry.get("processing_status") == "unprocessed":
                stats["existing_unprocessed"] += 1
            else:
                stats["unchanged_skip"] += 1
            if verbose:
                print(f"[EXISTS]  {source_path} (status: {entry.get('processing_status')})")
            continue
    ...
    # Salva ledger atualizado (somente se houver novas entradas ou alterações)
